...
tcgaout.batch <- doppelgangR(tcgaov.esets, phenoFinder.args=NULL, smokingGunFinder=NULL) 
tcgaout.nobatch <- doppelgangR(tcgaov.esets, phenoFinder.args=NULL, smokingGunFinder=NULL, 
                               corFinder.args=list(use.ComBat=FALSE))
@ 
...
\emph{outside} of this vignette.  The following code for preparing the ExpressionSets not run:
...
<<breastdoppel>>=
output <- doppelgangR(esets[c("UNT", "UPP")], outlierFinder.expr.args = list(bonf.pvalue=10, transFun=atanh, tail="upper"), outlierFinder.pheno.args=NULL)
output
