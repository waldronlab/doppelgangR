...
wuetal2 <- doppelgangR(eset2, outlierFinder.expr.args = list(bonf.prob = 40, transFun = atanh, tail = "upper"), 
                       phenoFinder.args = NULL, smokingGunFinder.args = NULL)

wuetal3 <- doppelgangR(eset3, outlierFinder.expr.args = list(bonf.prob = 40, transFun = atanh, tail = "upper"), 
                       phenoFinder.args = NULL, smokingGunFinder.args = NULL)
plot(wuetal3)

library(pheatmap)
pheatmap(exprs(eset2))
pheatmap(exprs(eset3))