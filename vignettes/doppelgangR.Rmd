...
results1 <- doppelgangR(testesets, phenoFinder.args=NULL, cache=TRUE)
```
...
par(mfrow=c(1,3), las=1)
nobatch.spearman.rocr <- with(tcgamelt.spearman.nobatch, prediction(cor, as.integer(truepos)))
withbatch.spearman.rocr <- with(tcgamelt.spearman.batch, prediction(cor, as.integer(truepos)))
tcga.remove <- read.delim(system.file("extdata", "TCGA_remove.txt",
                    package="doppelgangR"), as.is=TRUE)[, 1]
